  ...
  for (nomcolonne in bddplante) {
    ...
    for (params in bddplante[nomcolonne]) {
      ...
      if (params === 'valeurs' && !parametre.includes(checkValue.get(nomcolonne))) {
        console.log(
          `${nomcolonne} valeur accepté ${JSON.stringify(parametre)} valeur recu ${cleanedValue(
            checkValue.get(nomcolonne)
          )}`
        );
        errorMessage = `${nomcolonne} valeur accepté ${JSON.stringify(parametre)} valeur recu ${cleanedValue(
          checkValue.get(nomcolonne)
        )}`;
        res.write(errorMessage);
        return true;
